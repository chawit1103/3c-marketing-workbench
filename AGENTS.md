...
- M17 PR5 and final status correction updated docs/status/smoke only: README, AGENTS, Roadmap, Product Health Dashboard, M17 closeout report, and docs smoke. M17 and M18 are closed as GO WITH CONDITIONS. M19 PR1 propagated offline user assumptions, and M19 PR2 added configuration-only Simulation Configuration Workspace controls. M19 PR3 added a product-owned synthetic/offline platform engagement result model over the PR2 selected platforms/configuration. M19 PR4 adds a product-owned Executive Insight Dashboard over PR1 assumptions, the PR2 submitted configuration snapshot, and PR3 platform metrics. M19 PR5 adds a frontend-only Executive Decision Brief in the existing export review surface; M19 PR6 closes the program docs/status/smoke only. M20 PR1–PR3 SocialSense work is completed/merged. M20 PR4 is in progress on this branch as a fixture/offline aggregate-only 3C adapter integration; it is not merged, production-ready, live-platform-enabled, or a claim of live measurement. 3C uses only the SocialSense public SDK and preserves fail-closed behavior. Report/export redesign remains out of scope, PR6 closeout must not claim live measurement or SocialSense runtime consumption, and no M20 scope beyond the authorized PR4 integration is active. Do not add new workflows, runtime product changes, backend endpoints, SocialSense capability, live APIs, external services, persistence, auth, credentials, CRM/customer data, PII/private data, production automation, workflow redesign, workspace redesign, or MarketingSimulation changes.
...
- Approved top-level SocialSense public SDK allowlist for the PR4 fixture/offline aggregate adapter: `create_research_session`, `run_scenario`, `export_run`, and `load_domain_pack` imported directly from `socialsense`.
- The adapter may call `load_domain_pack('marketing')`, `create_research_session(...)`, `run_scenario(...)`, and `export_run(...)` only to produce the reviewed executable contract echo; no private/non-public `socialsense.*` module or call is allowed.
