...
REQUIRED_M17_CLOSEOUT_PHRASES = [
    ...
    "Recommendation: M17 closed as GO WITH CONDITIONS; prepare M18 kickoff next, do not implement M18 here.",
    "PR #27",
    "PR #28",
    "5cec77a26cffd5255d9051a2743ab20c79512607",
    "a03ecf236c638d69cd48c37465d70e03579ebc8f",
...
def main() -> None:
    ...
    m19_pr4_docs_by_path = {
        path: (ROOT / path).read_text(encoding="utf-8")
        for path in REQUIRED_M19_PR4_DOCS
        if (ROOT / path).is_file()
    }
    ...
    if (
        ...
        if m17_closeout_report_exists or m17_is_closed or "PR5 M17 validation/closeout complete" in combined_m17_text:
            ...
            stale_m17_phrases = [
                ...
                "pending this " + "final status correction",
                "this final-status correction branch " + "is pending",
                "pending " + "PR/merge",
                "pending " + "verification",
                "M17 closure " + "is pending",
                "PENDING" + " |",
            ]
            ...
            if "PR #28" not in m17_status_text or "a03ecf236c638d69cd48c37465d70e03579ebc8f" not in m17_status_text:
    ...
    if branch_at_or_after(19):
        ...
        elif current_branch_name().startswith("m19-pr4-"):
            combined_m19_pr4_text = "\n".join([
                readme,
                agents,
                roadmap,
                health_dashboard,
                ...
                (ROOT / "src/views.tsx").read_text(encoding="utf-8"),
                (ROOT / "src/App.test.tsx").read_text(encoding="utf-8"),
                (ROOT / "src/product/executiveInsights.ts").read_text(encoding="utf-8"),
                (ROOT / "src/product/executiveInsights.test.ts").read_text(encoding="utf-8"),
            ])
            missing_m19_pr4_phrases = [
                phrase for phrase in REQUIRED_M19_PR4_PHRASES if phrase not in combined_m19_pr4_text
            ]
